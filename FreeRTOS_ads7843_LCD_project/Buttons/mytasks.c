...
const char * const pcUsartTaskStartMsg = "USART tsak started.\r\n";
...
void vButtonCheckTask(void *pvParameters)
{
	...
	//create semaphore for each button
	...
	// if created then the value of semaphore handle would be 1
	if((xButtonWakeupSemaphore!=NULL)&&(xButtonTamperSemaphore!=NULL)
			&&(xButtonUser1Semaphore!=NULL)&&(xButtonUser2Semaphore!=NULL))
	{
		//It means that semaphores are created successfully
		//resets the semaphore initially to 0
